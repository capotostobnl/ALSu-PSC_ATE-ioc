...
<<<<<<< HEAD
    ...
    else if(oldPCFault2 != PCFault2){
        oldPCFault2 = PCFault2;
        if (PCFault2 == 1) sprintf(cmd, "F2\n"); // CH2 Set
        //else sprintf(cmd, "\n"); // CH2 Reset (Placeholder)
    }
    else if(oldPCFault3 != PCFault3){
        oldPCFault3 = PCFault3;
        if (PCFault3 == 1) sprintf(cmd, "F3\n"); // CH3 Set
